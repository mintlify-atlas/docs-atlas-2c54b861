...
| `200 OK` | `Validation` | `IDCompany` was missing or blank after trimming. |
| `200 OK` | `NotFound` | The specified company ID does not match any registered tenant. |

<Note>
  The Auth microservice always returns HTTP `200 OK` from this controller, even for validation and not-found conditions. Use the `success` and `errorType` fields in the response body to determine the outcome — do not rely on the HTTP status code alone.
</Note>

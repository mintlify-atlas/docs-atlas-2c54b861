---
title: "GSMAuth: Multi-Tenant Authentication Microservice"
...
<Steps>
  ...
  <Step title="Validate password hash and salt">
    Passwords are stored using PBKDF2. The submitted plaintext password is hashed against the stored `PasswordSalt` and compared to `PasswordHash`. A mismatch returns `401 Unauthorized`.
  </Step>
...
| `404 Not Found` | `IDCompany` not found or `IsActive = 0`. |
...
Passwords are never stored in plaintext. The `Users` table requires two columns:

- **`PasswordHash`** — the PBKDF2 output of the password combined with the salt.
- **`PasswordSalt`** — a randomly generated per-user salt.

<Warning>
  The business fields inherited from older schema versions are not sufficient for authentication. Both `PasswordHash` and `PasswordSalt` must be present in the `Users` table for the login flow to work. If either column is null for a user row, that user will be unable to authenticate.
</Warning>

## Environment Variables

<ParamField query="DB_MASTER_URL" type="string" required>
  Connection string to `TenantRegistryDb`. If not set, the service falls back to `ConnectionStrings:TenantRegistryConnection` in `appsettings.json`. In production this variable must be set and the `appsettings.json` value should be left empty or overridden.
</ParamField>

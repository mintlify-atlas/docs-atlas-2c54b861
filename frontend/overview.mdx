...
Each generator uses `mode: 'tags-split'` so hooks are grouped by OpenAPI tag into separate files. All hooks are wrapped by a custom mutator function defined in `src/shared/lib/fetcher.ts` that attaches credentials and handles auth errors globally.
...
| `vendor-ui` | `lucide-react`, all `@radix-ui/*` packages |
